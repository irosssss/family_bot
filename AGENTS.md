...
- **Роли V3**: папа/мама управляют семьёй и играют: свои дела, личные монеты и опыт.
  Собственное дело проверяет другой взрослый; единственный взрослый подтверждает его
  сам отдельным действием. За проверку чужих дел награды нет. См. `docs/V3_BASELINE.md`.
  В legacy-коде игровые действия ещё скрыты у родителей (`family_role !== 'parent'`),
  а target Player пока ограничен ребёнком: это долг реализации, не целевое правило.
  Не снимать защиту взрослого режима и не считать боссов/streak автоматически согласованными.
