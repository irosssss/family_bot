...
Чек-лист legacy-приложения (`server.ts` / `src/App.tsx`) для сервера с доменом и HTTPS.
Он не разворачивает новый `src/target/` и не означает готовность целевого MVP.
Локальная разработка (npm run dev) эти шаги не требует. Для отдельной временной демо
используйте [инструкцию preview](implementation/TELEGRAM_DEVICE_PREVIEW.md),
актуальный статус реализации — в [плане](IMPLEMENTATION_PLAN.md).
