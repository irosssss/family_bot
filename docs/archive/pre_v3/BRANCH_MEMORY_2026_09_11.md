...
Последнее обновление: 2026-09-09 (добавление V3 только в отдельной ветке)

## Изолированный третий вариант V3 — 2026-09-09

- Актуально после расширенного поручения «всю логику до ассетов»: 3.2–3.6/4 выполнены в `codex/family-life-v3`. `v3:local` на 127.0.0.1:3003 — отдельный сохраняемый synthetic PG/Express/UI; private ignored config 0600, project .env не читается. Старые ограничения «UI без API» ниже исторические. Реализованы calendar/tasks/settlement/ledger/corrections, quote/purchase/appearance/pets, child rewards/reserves/delivery, цель/adventure/trophies, PIN/invites/shared device/recovery. Game aggregate JSONB + семейная блокировка + отдельные immutable ledger/claims/receipts/CAS; перед production нужны нагрузочные/retention проверки.
- Финальный PG runner: 223/223 PASS (164 unit +59 PG/HTTP), миграции/rollback/concurrent migrate/sentinel/точный cleanup PASS; UI/model/transport/resolver32/32, lint/typechecks/build PASS. Browser375/390: детский return/resubmit/accept, purchase/apply, reserve/approve/delivery, hatch/corner, correction/restore, response-loss same-key replay без дубликата; сохранение после рестарта. Public visual IDs отделены от чужого кошелька/покупок. Документы: `docs/v3/PRE_ASSET_RESULT.md`, `RUNTIME_DESIGN.md`.
- Под ассеты готовы пустые v1-папки и `art/family-life-v3`; release entries0, PNG0. `GameAsset`/`GameHero` exact variants, profile/canvas/layer checks, placeholder при отсутствии/ошибке; `v3:assets:check` проверяет размеры/hash/учёт файлов. Формат `ASSET_DELIVERY.md`. Geometry candidate до артдирекции; следующий этап5 — пилот. Telegram SDK entry +HMAC verifier подготовлены, настоящий бот/HTTPS/WebView не подключены и не проверены. Main/Folio/меню бота/реальная БД не менялись; сезоны/платежи не входят в этот пакет.

### Историческое состояние после Foundation 3.1

- По отдельному «Продолжай» выполнен ограниченный этап3.1: `src/v3-server`, `migrations/v3`, schema `rpg_v3` только в собственной синтетической PostgreSQL18.6. Один Player-контракт для adult/child; grants отдельно от роли/Player, composite FK и уникальность member/player. Closed rawJSON/DTO, UUIDv7/revisions/bigint, opaque VerifiedActor и fixture adapter только tests/support. Guard перечитывает context под family→actor row locks; CAS revoke и probe в одной транзакционной границе. Paused Player запрещает self-effect, сохраняет own projection; admin/review взрослого зависит от grants active Member. Adult trusted-self — отдельный decision kind, manual self-review запрещён; реальные награды child-only eligibility. Managed-child/реальный вход/API/ledger/кошелёк/дела/покупки/награды/UI-to-DB отсутствуют.
- Этап3.1:177/177foundationtestsPASS (140unit+37PG), миграции/rollback/unique и обе lock-гонки с наблюдением PostgreSQL; точный cleanup+соседние sentinelPASS. Общийsuite419PASS/46opt-in skip;26V3UI/modelPASS;lint/typechecks/build/compiledimportsPASS. Сборка упаковывает проверенныйSQL рядом с migrator. Ревью исправило mutable revoke input (frozen DTO доawait) и family purchase безactiveличногоPlayer; PGregressionsPASS. UI остаётся этапом2, asset-free;375metricsбезoverflow/контролов<44, полного новогоWebViewQAнет. `docs/v3/STAGE_3_1_RESULT.md`; далее отдельная3.2послеcalendarwindow, settlement3.3. Main/Folio/бот/реальныеБД/ассеты не менялись.

- По отдельному поручению «изучи и добавь» подключены microsoft/tgrep и официальный Image Prompting GPT Image 2.5 только в V3. `scripts/v3/tgrep.mjs`, `v3:tgrep:install`/`v3:tgrep`: pinned 1.0.5 с SHA-256 macOS x64/arm64, бинарник/индексы в игнорируемом `work/v3-tools`. `rg` остаётся первым для разовых/свежих проверок; индекс явно ограничен `src/v3`, custom path передаётся index/serve/search/status. Локальная установка x64, parity 22 строки, no-match exit 1, server complete/via-server и Ctrl+C cleanup проверены; сервер остановлен. Детали `docs/v3/TOOLING.md`.
- `docs/v3/IMAGE_PROMPTING.md` и `templates/ASSET_PROMPT.md`: назначение/референсы/сохраняемая геометрия, отдельные слои/экспорты, QA 375/390, alpha и история вариантов. Flare/Sunburst указаны по документации 2026-09-09; builtin image_gen не имеет аргумента модели, GPT Image 2.5 не объявлять реально использованным без подтверждения. Изображения/API/ключи не задействованы; будущий этап графики не запущен.

- Пользователь поручил изучить два новых GDD/экономических документа, провести подробный аудит, начать по этапам отдельный третий вариант без ассетов и описать их будущую вставку. Затем явно подтвердил: родители тоже игроки. Это локальное изменение V3; main/Folio не изменены.
- `codex/family-life-v3`, `work/telegram-preview/family-life-v3`, entry `family-life-v3.html`, код `src/v3`, порт 3002. Свой Vite с `envDir:false/publicDir:false`; без старого demo/target transport, БД, Telegram bootstrap, persistence, API и игровых ресурсов. Селектор профиля — просмотр фикстур, не авторизация.
- Этап 1: пять экранов, состояния дел/ролей/совместных долей, составы 2/4/8, реестр 13 planned/candidate слотов, инспектор. Размеры и слои графики пока кандидаты. Генерация/копирование/подключение ассетов не выполнялись. Инструкция: `docs/v3/ASSET_INSERTION_GUIDE.md`.
- Аудит выявил несходящиеся темпы: формула 381095 XP до уровня 100, базовые 65 XP/день = 5863 активных дня; сезон 7360 XP; босс для 4 героев — 27,7 дня без Ultimate вместо 3–5. Gold-цен нет, награды двух источников различаются. CLI/тесты описывают гипотезы, не новый движок. На этапе1 родители как игроки не означали принятие Owner/Pool/Stars/self-approval/платных бонусов; политика собственных взрослых дел уточнена пользователем на этапе2 ниже.
- Итог: lint/build PASS, 13 V3-тестов PASS, CUA 375/390 — пять экранов без overflow/контролов меньше 44 px; все 13 инспекторов, состав 8 и профиль PASS. Production-network при полном проходе: только HTML/JS/CSS, нет API/графики/warn/error. Главный/Folio checkout чистые. QA ограничен desktop browser, не реальным Telegram. Отчёт `docs/v3/STAGE_1_RESULT.md`.
- Этап 2 выполнен после «делай». Пользователь отдельно выбрал: взрослый сам отмечает собственные дела, детские результаты подтверждает взрослый, реальные семейные награды пока только детям. Эти правила приняты; формулы/цены/каталог/цели/питомцы/процедуры остальных контрактов пока proposed v0.1. PRODUCT/BALANCE/COMMAND/UI и STAGE_2_RESULT находятся в `docs/v3/`.
- Чистые `src/v3/model/{balance,catalog}.ts`, offline `v3:simulate`, 15 основных состояний и калькулятор в «Разметке». Новая кривая 15642 XP до100: базовые65 XP/34 Gold за активный день, уровень100 на календарный337 при5+2; одежда60 Gold на2активный день. Малый каталог насыщается, устойчивая многолетняя экономика не заявлена. Pet target/goal/reward фиксируются до работы; none не получает Pet XP задним числом. Реальная награда резервирует до фактической выдачи, отмена до неё освобождает резерв без credit.
- Этап2: lint/typecheck/build PASS, 26 V3-тестов PASS; CUA375/390 все15основных состояний без overflow/контролов<44; network толькоHTML/JS/CSS, warn/error0. Реестр14слотов с pet.corner, графики нет. Dependency graph17V3модулей, API/DB/storage/auth/игровыеоперации отсутствуют. Fixture selector не авторизация, аналитическая дедупликация не гарантияSQL.
- Карточка `STAGE_3_CARD.md` закрыта в ограниченном synthetic объёме3.1,20проверок; результат выше. Операции внедряются дальше по подэтапам. Графика5, сезон6, платежи7. Основной прежний контракт ниже остаётся историей исходного варианта; его решения не мигрировать автоматически.
...
## V3: повторный аудит 2026-09-09

- Authenticated POST проверяет ожидаемый member против cookie actor: общая cookie другой вкладки не меняет получателя действия. UI читает согласованный bootstrap и отбрасывает устаревшие ответы.
- Завершённая цель не привязывается к новым работам; старые адреса и ledger сохраняются. UI учитывает сохранённую зону/срок работы и исчезновение записей при отзыве прав.
- Проверено:36 UI/model и225 server/PG, lint/typechecks/build, browser375/390. Подробности и исправление прежнего QA84/26: `docs/v3/REAUDIT_RESULT.md`. Реальный Telegram и графика остаются отдельной приёмкой.

## V3: художественный аудит 2026-09-09

- `docs/v3/ASSET_ARCHITECTURE_AUDIT.md`: 14 типов мест, 12 в LiveApp, 0 записей поставки; ориентировочно 39 variant/state-записей текущего охвата, не число PNG. Реестр не является готовым compositor: семья/мебель/комната и питомец/уголок пока отдельно. Нет индивидуального appearance preset.
- Референсы задают мягкий детальный cartoon/chibi; первая заставка — предпочтительный ориентир. `image-rendering:pixelated`, размеры оболочки ready/placeholder и формальная profile-валидация требуют исправления до приёмки реального арта. Размеры экспортов остаются кандидатами.
- `docs/v3/LOADING_SCREEN_SPEC.md` — спецификация, не реализованный загрузчик. Нужны HTML-оболочка, реальные состояния/повтор и чистый artwork без запечённой полосы. `IMAGE_PROMPTING.md` дополнен узкими заданиями и пилотом совместимости; генерация и изменение runtime в этом аудите не выполнялись.
