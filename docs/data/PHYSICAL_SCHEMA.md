...
## Разница между целевой V3 и реализованной основой · 16.09.2026

[V3_BASELINE](../V3_BASELINE.md) требует взрослого игрока для собственных дел, личных монет/XP и комплектов. `players.role CHECK child` ниже описывает прежний проект и реализованную основу G02; это известное расхождение, не целевое ограничение V3. Аналогично фраза о parent без player/кошелька в AppearanceSelection больше не задаёт целевую модель.

Расширение выполняется отдельной новой миграцией с проверкой FK/CHECK, family scope, однократного бесплатного комплекта, кошелька/XP и авторизации в G07/W21–W25 и G08/W26–W28. Применённые миграции не переписываются; реальные данные не переносятся этой фиксацией. Остальные PDB-требования, locks, ledger и ограничения уникальности сохраняются.
...
## V3: журнал согласий (16.09.2026)

Добавление0008: rpg.consent_events, append-only grant/withdraw; семейный FK субъекта, FK аккаунта действующего лица, назначение service/analytics, digest документа, последовательная версия области, reference доверенного доказательства, retention FK. Транзакционная блокировка области и expected revision предотвращают две записи одной версии. Старая версия документа и последний withdraw не дают действующего согласия. Реализация авторизации доказательств, очистки по политике сроков и HTTP onboarding остаётся отдельной работой; миграции0001–0007 не изменены.
