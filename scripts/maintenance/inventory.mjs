...
for(const p of [...names].sort()){
 ...
 else if(main.has(p)){category='active';basis='Current entry/server dependency; includes legacy compatibility and production branch';}
 else if(p.startsWith('src/target/')||target.has(p)||p.startsWith('migrations/target/')||p.startsWith('tests/target/')||p.startsWith('src/v3/access/')){category='prepared';basis='Target backend/content/access foundation; not fully connected to V3 UI';}
