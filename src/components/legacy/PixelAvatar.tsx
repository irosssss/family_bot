...
export interface PixelAvatarProps {
 ...
 icon?: string;
 emotion?: 'happy' | 'neutral' | 'sad' | 'excited' | 'tired';
}
...
export const PixelAvatar: React.FC<PixelAvatarProps> = ({
 ...
 // ==========================================
 if (type === 'pet') {
   // Используем spriteSheetUrl/icon питомца, если есть — иначе 16-bit JPEG fallback
   const petSprite = imageUrl || icon || '';
   if (petSprite) {
     return (
       <div className={`relative inline-flex items-center justify-center select-none ${animClass} ${className}`}>
         <div className="relative overflow-hidden bg-transparent flex items-center justify-center" style={{ width: sizePx, height: sizePx }}>
           <img src={petSprite} alt="" className="w-full h-full object-contain pixel-art [image-rendering:pixelated]" />
         </div>
       </div>
     );
   }
 ...
 // ==========================================
 if (type === 'boss') {
   const bossJpegUrl = icon || getBoss16BitJpeg(bossName || bossEmoji || fallbackEmoji || 'boss');
   return (
 ...
 const avatarLayers = get32BitAvatarLayers(dummyUser, equippedUrls, null, emotion);
