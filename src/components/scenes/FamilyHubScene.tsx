...
export const FamilyHubScene: React.FC<FamilyHubSceneProps> = ({
  ...
  return (
    <div className="relative w-full rounded-3xl overflow-hidden border-2 border-amber-500/30 bg-slate-950 shadow-2xl transition-all">
      {/* Real Game Background Image: /assets/game/home_bg.png */}
      <div 
        className="absolute inset-0 bg-cover bg-center bg-no-repeat [image-rendering:pixelated]"
        style={{ 
          backgroundImage: `url('/assets/game/home_bg.png')`,
      ...
      <div className="relative min-h-[360px] sm:min-h-[480px] p-4 sm:p-10 flex flex-col justify-end">
        {/* Family Members Standing Directly on Room Floor */}
        <div className="relative z-10 flex items-end justify-center gap-10 sm:gap-24 pt-12 pb-4">
          {appState.users.map((user) => {
            ...
            // Find equipped items for 32-bit layer renderer
            const equipped = user.equipped || {};
            const layers = get32BitAvatarLayers(user, equipped, null);
            ...
            return (
              ...
              >
                ...
                {isBubbleOpen && (
                  <div className="absolute -top-36 sm:-top-40 left-1/2 -translate-x-1/2 z-40 w-64 sm:w-80 p-3 sm:p-4 rounded-2xl bg-slate-900/95 border-2 border-amber-400 text-white shadow-2xl backdrop-blur-md animate-in fade-in zoom-in-95 duration-200">
                ...
                >
                  ...
                  <div className="hidden sm:block [image-rendering:pixelated]">
                    <LayeredAvatar layers={layers} size={140} animated={true} />
                  </div>
                ...
                {/* Integrated Pet Companion Standing/Floating Beside Character */}
                {petObj && (
                  <div className="absolute -right-8 sm:-right-12 bottom-6 sm:bottom-8 z-20 flex flex-col items-center animate-bounce">
                    <PixelAvatar
                      type="pet"
                      imageUrl={petObj.imageUrl}
                      fallbackEmoji={petObj.emoji || petObj.title}
                      size="sm"
                      animated={true}
                    />
                    {/* Tiny Pet Floor Shadow */}
                    <div className="w-6 h-1.5 bg-black/30 blur-[1px] rounded-full -mt-1 pointer-events-none" />
                  </div>
                )}
              </div>
            );
