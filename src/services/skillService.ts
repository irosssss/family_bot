...
export function applySkill(user: User) {
 ...
 message = `Мощный удар Воина! Нанесено 150 урона (-10 MP). БОСС ${appState.boss.emoji} ПОВЕРЖЕН! Вся семья получила +20!`;
 } else {
 message = `Мощный удар Воина! Босс получил 150 урона (${appState.boss.hp - appState.boss.damage}/${appState.boss.hp} HP) [-10 MP].`;
 }
 } else {
