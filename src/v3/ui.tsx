...
export function Meter({ value, max, label }: { value: number; max: number; label: string }) { max=Number.isFinite(max)&&max>0?max:1;value=Number.isFinite(value)?Math.max(0,Math.min(max,value)):0; return <div className="v3-meter" role="progressbar" aria-label={label} aria-valuemin={0} aria-valuemax={max} aria-valuenow={value}><span style={{ width: `${Math.max(0, Math.min(100, value / Math.max(1, max) * 100))}%` }}/></div>; }
