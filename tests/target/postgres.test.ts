...
describe.skipIf(!enabled)('task-owned PostgreSQL 18 (FT03/FT06–FT10)', () => {
  ...
  it('applies the reviewed bootstrap and repeats without effects', async () => {
    expect(await runTargetMigrations(sql, config, 'migrations/target')).toBe(8);
    ...
    expect(rows.map(row => row.name)).toEqual(['0001_bootstrap.sql', '0002_family_foundation.sql', '0003_identity_exchange.sql', '0004_family_access.sql', '0005_adult_protection.sql', '0006_access_lifecycle.sql', '0007_content_release.sql', '0008_consent_events.sql']);
